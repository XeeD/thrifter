...
Pokud /^typ dopravy "(.*?)" existuje$/ do |shipping_method_name|
...
Pokud /^jsem v editaci typu dopravy "(.*?)"$/ do |shipping_method_name|
  step "typ dopravy \"#{shipping_method_name}\" existuje"
  step 'jsem v sekci "administrace typů dopravy"'
  step "kliknu na řádku u typu dopravy \"#{shipping_method_name}\" na odkaz \"Upravit\""
end

Pokud /^typ dopravy "(.*?)" existuje a je (\d+)\. v pořadí$/ do |shipping_method_name, position|
  shipping_method = ShippingMethod.find_by_name(shipping_method_name)
...
# When statements
Když /^přesunu řádek typu dopravy "(.*?)" o (\d+) pozic(?:i|e)? (nahoru|dolů)$/ do |name_source, distance, direction|
...
Pak /^typ dopravy "(.*?)" by měl být (\d+)\. v pořadí$/ do |shipping_method_name, position|
