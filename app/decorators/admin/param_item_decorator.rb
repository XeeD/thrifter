  ...
  def input_check_boxes_tags
    h.content_tag :ul do
      param_item.param_values.map do |param_value|
        h.content_tag :li do
          input_check_box_tag(param_value.value, param_value.value)
        end
      end.join.html_safe + input_new_field_check_box_tag
    end
  end

  def input_check_box_tag(value, label)
    check_box = h.check_box_tag("param_items[#{param_item.id}][]", value, @assigned_param_values.fetch(param_item.id, []).include?(value))
    h.content_tag :label, check_box + label
  end
  ...
  def input_choice_tags
    ...
    # integer and String radio button choices
      ...
      h.content_tag :ul do
        param_item.param_values.map do |param_value|
          h.content_tag :li do
            input_radio_tag(param_value.value, param_value.value)
          end
        end.join.html_safe + input_new_field_radio_tag
      end
    end
  end

  def input_radio_tag(value, label)
    radio = h.radio_button_tag("param_items[#{param_item.id}]", value.to_s, @assigned_param_values[param_item.id] == value)
    h.content_tag :label, radio + label
  end
